...
class Schedule:
    ...
    def get_schedule_id_by_teacher(
        self, day: int, week: int, teacher_name: str
    ) -> list[int]:
        query = """
            SELECT s.ScheduleID
            FROM (
        ...
        if teacher_name in special_teachers:
            group_result = self.get_schedule_id_by_group(
                day, week, special_teachers[teacher_name]
            )
    ...
    def get_schedule_id_by_group(
        self, day: int, week: int, group_name: str
    ) -> list[int]:
        query = """
            SELECT s.ScheduleID
            FROM (
    ...
    def add_room(self, room_name: str) -> int:
        self.__cursor.execute(
            "SELECT RoomID FROM Rooms WHERE RoomName = ?", (room_name,)
        )
