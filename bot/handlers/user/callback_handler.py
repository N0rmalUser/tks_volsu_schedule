...
async def day_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    ...
    if keyboard_type == "teacher":
        ...
        if user.type == "teacher":
            week_kb = kb.get_days_teacher(callback.from_user.id, keyboard_type='teacher', week=week, value=value)
        else:
...
async def room_handler(callback: CallbackQuery, callback_data: ChangeCallbackFactory) -> None:
    ...
    await callback.message.edit_text(
        text_maker.get_room_schedule(day=user.day, week=user.week, room_name=Schedule().get_room_name(callback_data.value)),
...
async def teacher_handler(callback: CallbackQuery, callback_data: ChangeCallbackFactory) -> None:
    ...
    if user.type == "teacher":
        week_kb = kb.get_days_teacher(callback.from_user.id, keyboard_type='teacher', week=user.week, value=callback_data.value)
    else:
        week_kb = kb.get_days(callback.from_user.id, keyboard_type='teacher', week=user.week, value=callback_data.value)
...
async def group_handler(callback: CallbackQuery, callback_data: ChangeCallbackFactory) -> None:
    ...
    await callback.message.edit_text(text_maker.get_group_schedule(day=user.day, week=user.week,
                                     ...
                                     reply_markup=kb.get_days(callback.from_user.id, keyboard_type='group',
                                                              week=user.week,
...
async def schedule_editing_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    from bot.misc import text_maker_editing as tme
    teacher_id = callback_data.value

    user = UserDatabase(callback.from_user.id)
    text = tme.get_teacher_schedule(day=user.day, week=user.week, teacher_name=Schedule().get_teacher_name(callback_data.value))

    schedule = Schedule().get_schedules_id(day=user.day, week=user.week, teacher_name=Schedule().get_teacher_name(callback_data.value))
    if schedule is []:
        week_kb = kb.add_new_lesson(user_id=callback.from_user.id, value=teacher_id)
    else:
        week_kb = kb.get_editing(user_id=callback.from_user.id, value=teacher_id, schedule=schedule)
...
async def lesson_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    from bot.misc.text_maker import get_time_symbol

    keyboard = kb.get_editing_menu(schedule_id=callback_data.schedule_id,
                                   week=UserDatabase(callback.from_user.id).week,
                                   value=callback_data.value)
    time, day, week, subject, group, room, teacher = Schedule().get_schedule(schedule_id=callback_data.schedule_id)
    text = f"{day}       {week}\n{teacher}\n"
...
async def time_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    keyboard = kb.edit_time(schedule_id=callback_data.schedule_id,
                            value=callback_data.value)
...
async def time_edit_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    times = ['8:30', '10:10', '12:00', '13:40', '15:20', '17:00', '18:40']
    Schedule().edit_time(schedule_id=callback_data.schedule_id, time=times[int(callback_data.edit)])
...
async def day_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    ...
    keyboard = kb.edit_day(schedule_id=callback_data.schedule_id,
                           value=callback_data.value)
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()
...
async def day_edit_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
...
async def week_edit_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    Schedule().edit_week(schedule_id=callback_data.schedule_id, week="Числитель" if UserDatabase(callback.from_user.id).week == 2 else "Знаменатель")
    await schedule_editing_handler(callback, callback_data)


@router.callback_query(ScheduleEditingCallbackFactory.filter(F.action == "lesson_type"))
async def lesson_type_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
...
async def lesson_type_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    ...
    Schedule().edit_lesson_type(schedule_id=callback_data.schedule_id, lesson_type=callback_data.edit)
    await schedule_editing_handler(callback, callback_data)


@router.callback_query(ScheduleEditingCallbackFactory.filter(F.action == "teacher"))
async def teacher_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    keyboard = kb.edit_teacher(schedule_id=callback_data.schedule_id, value=callback_data.value)
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()
...
async def week_edit_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    ...
    Schedule().edit_teacher(schedule_id=callback_data.schedule_id, teacher=Schedule().get_teacher_id(callback_data.edit))
    await schedule_editing_handler(callback, callback_data)


@router.callback_query(ScheduleEditingCallbackFactory.filter(F.action == "group"))
async def group_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    keyboard = kb.edit_group(schedule_id=callback_data.schedule_id, value=callback_data.value)
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()
...
async def room_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:

    keyboard = kb.edit_room(schedule_id=callback_data.schedule_id, value=callback_data.value)
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()
...
async def add_new_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    pass


@router.callback_query(ScheduleEditingCallbackFactory.filter(F.action == "delete"))
async def delete_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
...
async def cancel_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    value = callback_data.value
    week = UserDatabase(callback.from_user.id).week

    text = text_maker.get_teacher_schedule(day=callback_data.day, week=week, teacher_name=Schedule().get_teacher_name(value))
    week_kb = kb.get_days_teacher(callback.from_user.id, keyboard_type='teacher', week=week, value=value)
...
async def back_handler(callback: CallbackQuery, callback_data: DayCallbackFactory) -> None:
    await schedule_editing_handler(callback, callback_data)
