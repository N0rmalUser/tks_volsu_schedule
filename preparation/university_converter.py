...
all_schedules = {}
files = os.listdir(ORIGINAL_SCHEDULES_PATH)
bar = Bar('Processing', max=len(files))

for filename in files:
    bar.next()
    if filename.endswith('.docx'):
        file_path = os.path.join(ORIGINAL_SCHEDULES_PATH, filename)
...
output_file_path = 'preparation\\university.json'

with open(output_file_path, 'w', encoding='utf-8') as output_file:
    json.dump(all_schedules, output_file, ensure_ascii=False, indent=4)
print(f'\n\nОбщий JSON-файл сохранен в файле: {output_file_path}\n')
